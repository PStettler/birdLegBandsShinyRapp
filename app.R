    ...
    output$main_content <- renderUI({
      if (current_view() == "info") {
        ...
        wellPanel(
          ...
          h5("Trouble shooting:"),
          tags$ul(
            tags$li(tags$b("Data loading error:"), " The program is designed to automatically detect the required coulmns in your data. The provided file must contain a column with at least one column stating the band colors. As a rule of thumb, if the data in the file can be interpreted by a human, this programm should be able to so too. If the program fails anyways, try to provide the data in an 'easier' format (e.g. one column with left identifiers, one with right identifiers)"),
            tags$li(tags$b("Color not known:"), " The program runs on a database of ~150 color names in english, german, and french language. Nevertheless, a color might be unknown to the program because of writing differences or other reasons. The unknown color name will be displayed in the error message at the bottom right of the screen. Spell this color differently in the provided dataset or replace it with a name of a similar color."),
            tags$li(tags$b("File input problems:"), " This program allows various input file types (.xslx, .xls, .csv, ...). However, errors may occour with some data file types, especially if the input data contains special characters and the encoding type does not match the expected data encoding type. This program is robust, but cannot do magic. It is commended to upload data from .xlsxs or .csv files to not risk encoding problems."),
          ),
          hr(),
